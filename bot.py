...
# Популярные города по твоему списку
...
def should_send_today(days_string):
    if days_string is None:
        return True  # Для старых записей без дней считаем как ежедневно
...
def get_cities_keyboard():
    ...
    # Добавляем кнопку для поиска другого города
...
def get_time_keyboard(city):
    ...
    # Добавляем кнопку для своего времени
...
def get_manage_reminders_keyboard(user_id):
    ...
    if not reminders:
        # Возвращаем клавиатуру только с кнопкой "Назад"
        keyboard.add(InlineKeyboardButton("🔙 Назад в меню", callback_data="back_to_menu"))
        return keyboard
    ...
    for reminder in reminders:
        ...
        # Проверяем, что days не None
        if days is None:
            days = "everyday"

        # Формируем текст дней
        if days == "everyday":
            days_text = "ежедневно"
        elif days == "workdays":
            days_text = "будни"
...
def handle_main_keyboard(message):
    if message.text == "🌤 Узнать погоду":
        ...
        if reminders:
            ...
            # Отправляем сообщение с клавиатурой, которая содержит кнопку "Назад"
        ...
        # Проверяем, не ввел ли пользователь время для напоминания
        user_id = message.from_user.id
        if user_id in user_data and user_data[user_id].get('awaiting_time'):
            ...
            # Если пользователь ввёл название города напрямую
...
def handle_callback(call):
    if call.data.startswith("city_"):
        ...
        # Сохраняем город и время, переходим к выбору дней
        ...
        # Сохраняем город и устанавливаем флаг ожидания времени
    ...
    # Убираем "часики" после нажатия на кнопку
